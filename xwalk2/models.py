...
class WalkDefinition(BaseModel):
    image: str
    audio: str
    duration: float
...
class APIResponse(BaseModel):
    ...
    walk_queue: List[str] = Field(
        default_factory=list, description="List of queued walk animations"
    )
    walk_history: List[Tuple[datetime, str]] = Field(
        default_factory=list, description="History of walks with timestamps"
    )
...
class PlayScene(BaseModel):
    ...
    intro: WalkDefinition
    walk: WalkDefinition
    outro: WalkDefinition
    stop: WalkDefinition
